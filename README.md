...
<style>
/* Night blue background for entire README */
body {
  background: linear-gradient(135deg, #0f1419 0%, #1a2332 100%) !important;
  color: #c1d2f0 !important;
  font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif !important;
}

/* Markdown container - centered content */
.markdown-body {
  background: linear-gradient(135deg, #0f1419 0%, #1a2332 100%) !important;
  color: #c1d2f0 !important;
...
/* Headers with night blue theme - centered */
h1, h2, h3, h4, h5, h6 {
...
/* Paragraphs centered with spacing */
p {
  text-align: center !important;
...
/* Tables - centered */
table {
  background: rgba(30, 42, 58, 0.2) !important;
...
td {
  background: rgba(30, 42, 58, 0.1) !important;
  color: #d1d5db !important;
  border: 1px solid #374151 !important;
...
/* Special alignment for feature cards table */
td h3 {
  text-align: center !important;
...
/* Details/Summary elements - centered */
details {
  background: rgba(30, 42, 58, 0.2) !important;
  border: 1px solid #374151 !important;
...
/* Code blocks styling - left aligned */
pre {
  background: linear-gradient(135deg, #0f1419 0%, #1a2332 100%) !important;
  border: 1px solid #2d3748 !important;
  border-radius: 8px !important;
  padding: 20px !important;
  box-shadow: 0 4px 16px rgba(15, 20, 25, 0.4) !important;
  position: relative !important;
...
pre::before {
  ...
  background: linear-gradient(90deg, #4c6ef5, #339af0, #228be6) !important;
...
/* Enhanced TypeScript/JavaScript syntax highlighting inspired by ts-morph with night blue theme */
/* Comments */
.hljs-comment { color: #6b7280 !important; font-style: italic !important; }
/* Keywords: const, let, var, function, etc. */
.hljs-keyword { color: #60a5fa !important; font-weight: 600 !important; }
/* Strings and import paths */
.hljs-string { color: #fbbf24 !important; }
/* Function names: React.lazy, dynamic, lazy */
.hljs-title, .hljs-function > .hljs-title { color: #34d399 !important; font-weight: 600 !important; }
/* Variables and identifiers */
.hljs-variable, .hljs-name { color: #a5b4fc !important; }
/* Numbers */
.hljs-number { color: #c084fc !important; }
/* Operators: =, =>, etc. */
.hljs-operator { color: #e5e7eb !important; }
/* Punctuation: (), {}, [], etc. */
.hljs-punctuation { color: #e5e7eb !important; }
/* Types: React.ComponentType */
.hljs-type { color: #22d3ee !important; font-weight: 500 !important; }
/* Property access: React.lazy */
.hljs-property { color: #a5b4fc !important; }
/* JSX/TSX specific */
.hljs-tag { color: #60a5fa !important; }
.hljs-attr { color: #7dd3fc !important; }
.hljs-attr-value { color: #fbbf24 !important; }

/* Manual syntax highlighting for our specific code patterns */
pre code {
  /* Base text color */
  color: #c1d2f0 !important;
}
/* Highlight specific patterns in our code examples */
pre code:not(.hljs) {
  /* Since we might not have hljs, let's manually style common patterns */
  background: transparent !important;
  font-family: 'JetBrains Mono', 'Fira Code', 'Cascadia Code', Consolas, monospace !important;
}
</style>

<!-- Manual syntax highlighting for better code appearance -->
<script>
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.querySelectorAll('pre code').forEach(function(block) {
    ...
    // Keywords
    html = html.replace(/\b(const|let|var|function|import|export|default|async|await)\b/g, '<span style="color: #60a5fa; font-weight: 600;">$1</span>');
    
    // React/Next.js functions
    html = html.replace(/\b(React\.lazy|dynamic|lazy)\b/g, '<span style="color: #34d399; font-weight: 600;">$1</span>');
    
    // Strings (import paths)
    html = html.replace(/(["'])([^"']*)\1/g, '<span style="color: #fbbf24;">$1$2$1</span>');
    
    // Comments
    html = html.replace(/(\/\/.*$)/gm, '<span style="color: #6b7280; font-style: italic;">$1</span>');
    
    // Variable names (before =)
    html = html.replace(/\b([A-Za-z_$][A-Za-z0-9_$]*)\s*=/g, '<span style="color: #a5b4fc;">$1</span>=');
    
    // Types
    html = html.replace(/:\s*([A-Za-z_$][A-Za-z0-9_$.<>]*)/g, ': <span style="color: #22d3ee; font-weight: 500;">$1</span>');
    
    // Arrow functions
    html = html.replace(/=>/g, '<span style="color: #60a5fa; font-weight: 600;">=></span>');
    
    // Object properties (ssr: false)
    html = html.replace(/(\w+)(\s*:\s*)(false|true|null|undefined|\d+)/g, '<span style="color: #7dd3fc;">$1</span>$2<span style="color: #60a5fa;">$3</span>');
...
### 🎯 **The Missing Piece for React & Next.js Developers**
